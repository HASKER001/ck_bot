...
# --- НАСТРОЙКИ ---
# ==========================================
# Токен теперь берется из переменных окружения Render
TOKEN = os.environ.get("VK_TOKEN")
GROUP_ID = "240126179" 
ADMIN_ID = 829383686  # Твой цифровой ID (Владелец)

if not TOKEN:
    print("ОШИБКА: Переменная окружения VK_TOKEN не задана на сервере!")
    exit(1)
...
# ==========================================
# --- ЗАГЛУШКА FLASK ДЛЯ RENDER И CRON-JOB ---
# ==========================================
app = Flask(__name__)

@app.route('/')
def home():
    return "Бот Black Russia Family работает в штатном режиме!"

def run_flask():
    # Render автоматически задает переменную PORT
    port = int(os.environ.get("PORT", 8080))
    # Запускаем Flask на всех сетевых интерфейсах (0.0.0.0), чтобы Render мог до него достучаться
    app.run(host='0.0.0.0', port=port)

def keep_alive():
    # Создаем отдельный поток для Flask, чтобы он не блокировал LongPoll
    server = threading.Thread(target=run_flask)
    server.daemon = True # Поток завершится при остановке основного скрипта
    server.start()
...
if __name__ == "__main__":
    ...
    # 3. Запуск LongPoll ВК
    longpoll = VkBotLongPoll(vk_session, GROUP_ID)
    print("Бот Black Russia Family успешно запущен! Flask-сервер активен.")
